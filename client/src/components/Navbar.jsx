...
export default function Navbar() {
  ...
  return (
    <>
      <nav
        className={`fixed top-0 right-0 left-0 z-50 transition-all duration-300 ${scrolled ? "bg-[#0a0a0f]/95 backdrop-blur-md border-b border-white/5 shadow-xl" : "bg-transparent"}`}
      >
        <div className="container mx-auto px-6 h-16 flex items-center justify-between">
          <Link to="/" className="flex items-center gap-2">
            ...
            />
            <span className="font-bold text-base">ميكا بورتفوليو</span>
          </Link>
      ...
      >
        ...
        {/* Scrollable body */}
        <div className="flex-1 overflow-y-auto px-5 py-6 space-y-8 duration-900 ease-in-out">
          {/* Navigation links */}
          <div>
            <p className="text-slate-600 text-xs font-medium mb-4 text-right">
              التنقل
            </p>
            <div className="space-y-1">
              {topLinks.map((l) => (
                <a
                  ...
                  href={l.href}
                  onClick={close}
                  className="flex items-center justify-start w-full py-3 px-2 text-slate-200 hover:text-white rounded-xl hover:bg-white/5 transition-all group gap-3"
                >
                  <i
                    className={`fa-solid ${l.icon} text-slate-600 group-hover:text-blue-400 text-md transition-colors`}
                  />
                  <span className="text-l font-semibold">{l.label}</span>
                </a>
              ))}
            </div>
          </div>
