...
const HERO_MODELS = [
  ...
  // free_sci-fi_vehicle — futuristic sci-fi vehicle
  { url: '/models/free_sci-fi_vehicle/scene.gltf', scale: 1.6,  posY: -0.4 },
  // toon_drone  — stylised toon quad-drone
  { url: '/models/toon_drone_-_4m-ru/scene.gltf', scale: 1.8,  posY: -0.35 },
];
...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen text-white font-arabic" style={{ background: '#0b0f1a' }} dir="rtl">
      <Navbar />
      ...
      {/* ─────────────────── Featured Projects ─────────────────── */}
      <section id="projects" className="py-20" style={{ background: '#0e1320' }}>
              ...
              <div className="flex flex-wrap gap-3">
                {profile?.linkedin && (
                  <a href={profile.linkedin} target="_blank" rel="noreferrer"
                    ...
                    <i className="fa-brands fa-linkedin text-blue-400" /><span>LinkedIn</span>
                  </a>
                )}
                {profile?.whatsapp && (
                  <a href={`https://wa.me/${profile.whatsapp}`} target="_blank" rel="noreferrer"
                    className="flex items-center gap-2 bg-[#111827] hover:bg-[#1a2235] px-4 py-2 rounded-lg border border-white/5 transition-colors text-sm">
                    <i className="fa-brands fa-whatsapp text-green-400" /><span>واتساب</span>
                  </a>
                )}
                {profile?.email && (
                  <a href={`mailto:${profile.email}`}
                    className="flex items-center gap-2 bg-[#111827] hover:bg-[#1a2235] px-4 py-2 rounded-lg border border-white/5 transition-colors text-sm">
                    <i className="fa-solid fa-envelope text-slate-400" /><span>{profile.email}</span>
                  </a>
                )}
              </div>
            </div>
            ...
            <div id="skills">
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  <div key={i} className="flex items-center gap-3 bg-[#111827] rounded-xl p-4 border border-white/5 hover:border-blue-500/30 transition-colors">
        ...
        <div className="container mx-auto px-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
            {categories.map((cat, i) => (
              <div key={i} className="flex flex-col items-center gap-3 bg-[#111827] rounded-xl p-6 border border-white/5 hover:border-blue-500/30 hover:bg-[#162035] cursor-pointer transition-all group">
      ...
      <section id="contact" className="py-20" style={{ background: '#0b0f1a' }}>
        <div className="container mx-auto px-6 max-w-2xl">
          ...
          <form onSubmit={handleContact} className="bg-[#111827] rounded-2xl p-8 border border-white/5 space-y-5">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm text-slate-400 mb-2">الاسم</label>
                <input type="text" value={contact.name}
                  onChange={e => setContact(p => ({...p, name: e.target.value}))} required
              ...
              <div>
                ...
                <input type="email" value={contact.email}
                  onChange={e => setContact(p => ({...p, email: e.target.value}))} required
                  className="w-full bg-[#0b0f1a] border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-blue-500 transition-colors"
            ...
            <div>
              ...
              <input type="text" value={contact.subject}
            ...
            <div>
              ...
              <textarea value={contact.message}
                onChange={e => setContact(p => ({...p, message: e.target.value}))} required rows={5}
            ...
            {contactStatus === 'success' && (
              <div className="flex items-center gap-2 text-green-400 bg-green-400/10 rounded-lg px-4 py-3">
                <i className="fa-solid fa-check-circle" /><span>تم إرسال رسالتك بنجاح!</span>
              </div>
            )}
            {contactStatus === 'error' && (
              <div className="flex items-center gap-2 text-red-400 bg-red-400/10 rounded-lg px-4 py-3">
                <i className="fa-solid fa-circle-exclamation" /><span>حدث خطأ، يرجى المحاولة مرة أخرى</span>
              </div>
            )}
            <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 rounded-lg transition-colors flex items-center justify-center gap-2">
              <i className="fa-solid fa-paper-plane" /><span>إرسال الرسالة</span>
            </button>
          </form>
        </div>
      </section>
